...
| Live CRM connections (Lead Docket / Filevine / Litify / Clio) | Real credentials + live create-only writes through the Phase-7 connector port. First candidate: Lead Docket direct (per-firm self-generated API key; status flip to "Rescued — Review" + note) feeding the rescue desk shipped 2026-07-11 — see `ops/drafts/rescue-crm-integration-ladder.md`. Until signed off, the rescue hand-back is a CSV the firm imports themselves. | Firm-scoped auth + per-integration approval required; mock CRM only until then. |

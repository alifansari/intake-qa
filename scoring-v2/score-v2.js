...
export async function scoreV2({ transcript, callId, firmConfigPath = DEFAULT_CONFIG }) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error("ANTHROPIC_API_KEY is missing from .env");

  const systemPrompt = readFileSync(join(PKG_ROOT, "system-prompt.md"), "utf8");
  const configText = readFileSync(firmConfigPath, "utf8");
  const { partA, partBYaml } = splitFirmConfig(configText);
  if (!partA) throw new Error(`No PART A block found in ${firmConfigPath}`);

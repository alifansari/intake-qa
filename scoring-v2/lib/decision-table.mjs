...
import { caseTypeRouting } from "./casetypes.mjs";
import { rankDevelopActions } from "./develop-actions.mjs";
import { boringPrior } from "./priors.mjs";
import { projectedNetLienTier } from "./liens.mjs";
...
export function buildReferComparison(dims, tiers, vTier, referDefault = false) {
  const effort = tierOf(tiers, "effort_tier");
  const surfaced =
    (vTier === "high" && effort === "heavy_lit") ||
    level(dims, "case_type_fit") === "fatal" ||
    // v2.2: case types that default to refer-out (med-mal, product, elder-
    // abuse heightened, comp-only) always surface the per-hour comparison —
    // refer-out is a first-class ROI-positive disposition, not a failure.
    referDefault === true;
...
export function decideDisposition({ llmOutput, gates, config }) {
  ...
  // --- case-type routing overlay (v2.2) ------------------------------------
  // Per-case-type defaults (med-mal/product/elder-abuse/comp-only → refer;
  // government/trucking → urgency; trucking → spoliation). This NEVER forces a
  // decline and respects the firm's accepted list (an accepted refer-default
  // type is kept and merely surfaces the refer comparison). The one behavioral
  // nudge: a refer-default type that reached sign_now on a HEAVY-effort file is
  // the per-hour case for refer-out — surface it as the recommendation.
  const routing = caseTypeRouting(caseType);
  if (routing.refer_default && recommended === "sign_now" && tierOf(tiers, "effort_tier") === "heavy_lit") {
    recommended = "refer_out";
    basis.push(
      `R-CT ${caseType} is a refer-default type on a heavy-effort file → refer_out (likely per-hour winner; ${routing.notes || "specialist value"})`
    );
  }

  const vTier = valueTier(dims, gates);
  const developDimTriggers = Object.keys(dims).filter((d) =>
    ["unknown", "thin"].includes(level(dims, d))
  );
  const ranked_actions =
    recommended === "develop"
      ? rankDevelopActions({
          facts,
          unknownDims: developDimTriggers,
          gateFlags: gates.flags,
        })
      : [];
  const develop_payload =
    recommended === "develop"
      ? { ...buildDevelopPayload(dims, tiers, posture), ranked_actions }
      : null;
  const refer_comparison = buildReferComparison(dims, tiers, vTier, routing.refer_default);

  // Anti-base-rate-neglect: the firm's boring prior, tier-only, labeled
  // published-not-firm-data. Stands NEXT TO the vivid call, never decides.
  const prior_context = boringPrior({ caseType, valueTier: vTier });

  // Lien context (tier only) when the richer lien fact was stated.
  const lienSrc = facts.lien_sources;
  const lien_context =
    lienSrc && actionable(lienSrc, false)
      ? projectedNetLienTier({ lien_sources: lienSrc })
      : null;
  ...
  return {
    ...
    case_type: caseType,
    case_type_routing: {
      refer_default: routing.refer_default,
      urgency: routing.urgency,
      spoliation: routing.spoliation,
      coverage_floor_source: routing.coverage_floor_source,
      note: routing.notes,
    },
    prior_context,
    lien_context,

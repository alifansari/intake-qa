...
| `/studio/rescue` | **The Rescue desk** — the layer above the firm's CRM cadence | Upload the CSV of leads the firm's CRM marked dead (Lead Docket Lost/Rejected/chase-complete, CloudLex export, etc.). Deterministic triage re-reads them on legal merit — statute clock, case type, why follow-up stopped — and the live ones queue for YOUR named confirmation, then flow into the same top-3 callback packet + RSQ ledger as call flags. Hand the list back as a tagged "Rescued — Review" CSV the firm imports into their CRM. Screened-out rows are kept with reasons (the honest denominator). No dollars anywhere — value is a tier with its cited basis. |

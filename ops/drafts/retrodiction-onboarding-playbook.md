...
> **Status:** staged operational playbook. Operationalizes Increment 0's retrodiction bulk
...
- **CASEpeer** (best case — PI-native): Ask for the **Closed Cases Report** (all closed-status cases in a date range; includes Total Settlement, Total Fee, Total Medical Bill; CSV via the Export button), plus the **Settlement Report** (accepted offers, accepted + deposit dates) and **Demand Report** (demand history from the Settlement tab) — both Excel/CSV. Have an **admin** run them; non-admins only see their own cases.
- **Clio Manage**: **Matters > "Closed" filter > Export > CSV** gives dates/practice area/status/SOL — *no dollars*. If the firm has the **Personal Injury add-on**, settlement, fees, liens, and net-client-compensation live on the matter's Settlement subtab, but no firm-wide closed-matters-with-settlement export is documented — **have the firm ask Clio support for a custom report** combining close date + settlement fields, or use the admin-only bulk export (app.clio.com/export) plus their disbursement records.

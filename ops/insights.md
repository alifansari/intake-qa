...
## 2026-07-20 — Attorney's-eyes product teardown → retire the retrospective-scorer-as-hero  ·  lanes: product, strategy, GTM  ·  verified research + strategy synthesis

Five parallel deep-research passes (PI intake economics, attorney frustrations, competitor/AI-distrust, unmet-needs/wishlist) + a full internal product-surface audit + the persona/insights already in this ledger. The ask: analyze the *product* (not the copy) as a CA-PI attorney would, and name what he dislikes and wants added. Sources tiered inline; the vendor stats are directional, the ABA/Clio/MIT/ATRA/primary-rule anchors are hard.

- **THE CORE FINDING (strategy):** the product is built on a correct idea — *actuarial beats clinical, so an independent, cited SCORE is the value* — that is **commercially inverted for this buyer.** The attorney does not experience value as a better judgment; he experiences it as **a saved case, a calmed fear, and a number he can defend to himself.** The product's center of gravity (retrospective · independent · scored · dollar-denominated) is the opposite of what moves him (immediate · embedded in workflow · actioned to a signed retainer · denominated in *certainty*, not dollars-at-risk). Every top objection is a symptom of that one inversion. → Ali's call 2026-07-20: **formally retire the retrospective-scorer-as-the-face positioning** (decisions.md 2026-07-20; backlog B-021…B-032).
- **The buyer, distilled (verified persona + research):** owns a 2–10-atty CA PI firm; self-image = trial lawyer/rainmaker; delegated the highest-leverage moment in every case to a ~$45k, ~15-mo-tenure, high-churn "answers-the-phone" seat. Five governing emotions: (1) marketing-money anxiety + **attribution blindness** (~42% of firms don't track marketing ROI — CallRail 2025; "everything after the form is invisible" — NatLawReview); (2) **SOL/malpractice terror** (missed statute = malpractice judgment for the whole case value — the load-bearing fear); (3) **vendor-burn cynicism** (shared junk leads, can't-exit contracts, "great in the trial period, worse once you pay"; 65% of B2B buyers call vendor ROI math inflated); (4) **trial-lawyer ego vs intake-as-cost-center** ("glorified answering service… an afterthought delegated to untrained staff" — Miami-Dade Bar 2026); (5) **post-ABA-Op-512 AI paranoia** (Rule 1.6 informed consent "not boilerplate"; vendor-vetting is now HIS ethical duty — CA/FL/NY Op 2025-6). Behavior: gives a tool ONE 20–30 min session then <5 min/wk; shelfware within 12 mo without a champion + day-1 ROI. One tailwind: contingency-fee ⇒ efficiency = money (the billable-hour objection doesn't bite).
- **What he DISLIKES about the actual product (ranked by kill-speed):** ① **it's the autopsy, not the save** — the flagship `lost_signable_case` + `$revenue_at_risk` lands post-call, after the caller signed with the first responder (78% hire first firm to have a real conversation; 5-min = 21× — MIT/Oldroyd); a next-day dollar-stamped statement of losses trips his vendor-ROI cynicism. ② **grade-as-surveillance** on the lowest-paid/highest-churn seat (HBR: monitoring raises rule-breaking unless perceived fair) — accelerates the turnover that IS his real problem. ③ **"my paralegal already does this"** — a pure grader is the weakest "so what" in the category. ④ **already built, trust-layer solved better** — Lawmatics QualifyAI (Oct 2025) shipped score+reasons+confidence as a *next action* ("Chase Hard/Chase/Refer/Reject") INSIDE the CRM, no double-entry; Lead Docket Predictive does sign/refer/reject. ⑤ **it doesn't DO anything & makes him do more** — nothing sends (all TEST_MODE/mock), only a mock CRM connector, hand-back is a **CSV he re-imports by hand** (double-entry = #1 integration grievance). ⑥ **a $ figure on a lost case** is exactly the inflated-vendor move he distrusts (and ironically the dollar-free, cited v2 is the hidden one). ⑦ **bar-complaint/malpractice fear** — records CA calls (CIPA §632), feeds transcripts to an LLM (Op 512), makes sign/decline calls (the "$10M case the AI turned away" cuts both ways); SOC2/BAA/no-training/right-to-audit are roadmap-labeled, not real. ⑧ **"which product am I buying?"** — QA + live triage + rescue + Mirror + chat demo + visible nav sediment; buyers want ONE job done undeniably well. ⑨ **rescue = "text my dead leads?"** — TCPA §227(b) + Rule 7.3 (PA banned text-solicitation Nov 2024) is his scariest mechanism. ⑩ **zero peers, never run on a real firm** — he trusts CAALA peer names + run-it-on-MY-calls above all.
- **What he WANTS ADDED (ranked; ✱ = cockpit-spec already anticipates):** 1) ✱ **save the case live/within minutes** (flag before the caller signs elsewhere) — live in-call guidance for a *human* specialist is genuine whitespace (65% of contact-center agents want real-time help — Cresta 2024; legal has only post-call scorecards or full-replacement bots). 2) ✱ **turn the verdict into the close** — e-sign on the call (now "industry baseline"), book the appt, fire follow-up; the paid deliverable is a signed retainer. 3) **native two-way CRM write-back** (Filevine/Litify/CASEpeer/SmartAdvocate/Lead Docket/Clio/CloudLex) — kill the CSV. 4) **marketing-source → SIGNED-case attribution** — his loudest analytics pain; raw material already in `call_analyses`. 5) **sell certainty not insight** — reframe SOL/never-drop + audit trail as "nothing slips, here's the proof." 6) **after-hours "which no's were signable"** wedge (not competing with saturated answering services). 7) **real full-conversation Spanish** intake+rescue (<12% of firms have it; CA ~40% Hispanic; *confianza* = #1 driver). 8) **explainability/override/calibration as the HEADLINE** (the anti-QualifyAI edge — deterministic, cited, calibratable, human-only declines; CalibrationDeck exists). 9) **proof on his terms** (run-on-my-last-50-calls demo + named peers).
- **THE UNCONTESTED MOAT (keep building the product around this):** the **survivorship-free record of which declines were WRONG.** CRMs score, answering services answer, voice agents capture — **none own the measured outcome of the cases a firm turned away.** It's a data moat + a category ("the firm that tells you which of your no's were signable") AND the one framing that turns the scorer from *surveillance* into *found money*. Already seeded as B-016 (conveyor Increment 0 / decline-capture spine) — connect the whole product to it.
- **Competitive clock (verified):** Lawmatics QualifyAI + Lead Docket Predictive + Filevine LeadsAI (black-box scoring, in-CRM); Eve "Jenny" + EvenUp Voice Agent (AI voice closers, Oct 2025, PI-trained) — the voice lane has 2 funded incumbents AND a deliberate refusenik (Lawmatics' Spiegel: *"Lawyers don't want some robot talking to their lead"*). CallRail CI scores *engagement not merit* ("can't tell you if the case is worth $50k or $5M") — the cleanest wedge for a PI-merit engine.
- **Source:** internal product-surface audit (2026-07-20, agent, full file-path inventory) + 5 web-research passes (ABA Op 512 PDF; Clio 2025 Legal Trends; MIT/Oldroyd 2007; ATRA 2020–24 ad-spend $2.64B; Rankings.io $284 CPL/7%/$468 CPSC; LawNext/Lawmatics QualifyAI + Eve/Jenny primary; Cresta 2024 real-time survey). Vendor stats flagged directional per §IV.
- **Confidence:** product-surface = verified (code-read); attorney-mind anchors = verified where ABA/Clio/MIT/ATRA/primary-rule, directional where vendor; the "inversion" thesis = STRATEGY (high-conviction, grounded).
- **Affects:** product (primary), strategy, GTM, website (positioning — but any public copy change still runs the §V/§VII pre-ship gate).
- **Implied hypotheses:** → B-021…B-032 (below), anchored on B-016 (the decline-capture moat).
- **Honest caveats (§VIII):** (a) most of what neutralizes his objections — real integrations, real sending, SOC2/BAA, a live cockpit on his phone system, one vouching peer — is **roadmap, not product**; from his chair that's the gap between a clever demo and a tool he'd bet his intake on. (b) Retiring the *retrospective* face does NOT retire the flat-fee/not-a-fee-participant compliance stance (§I) — see the decision entry for the boundary. (c) Tension worth Ali's eye: the "independent-scorer authority/benchmark" GTM asset (B-006/B-001) rests on *independence*, while the product is becoming *embedded + acting* (writes back, firm's staff text firm's leads). Those are now two identities — defensible as an independent published INDEX + an embedded PRODUCT, but no longer one seamless story. Downstream regulated slices (automated send, embedded routing) keep their existing Yang gates (cockpit-spec slice 4; §VII).

...
## 2026-07-20 — INTEGRATION B-023: ship Filevine intake write-back (dormant, live-gated) + CASEpeer verdict · agent: main session · lane: product [§VII per-integration]
- **Change (shipped, dormant):** shipped the full Filevine intake write-back as one reviewed unit — most of it was already built in the uncommitted pile; I reviewed it, added safety, and committed only the Filevine feature (not the rest of the pile). Files: `integrations/filevine.mjs` (real adapter — Bearer PAT + x-fv-orgid/x-fv-userid, Person→Project→Note moat), `src/lib/desk/intake-file.mjs` (SALI-coded record builder, §IV-clean), `src/app/api/desk/filevine/push/route.ts` (auth, firm-scoped, stages when unconnected), the connector `intake.created` case, and `tests/filevine-intake.test.mjs`. My additions: the live-gate on `fvReq` (simulate unless INTEGRATIONS_LIVE — a connected firm is never written to by accident) and honest note-methods (require a Filevine projectId, else skip — the placeholder posted to a nonexistent `/core/projects/notes`). +2 tests (690/690), build+lint green. Depends only on prod code (triage-view, firm.ts, store.getFirmIntegration) — no other pile pieces pulled in.
- **CASEpeer verdict:** NOT buildable as a REST adapter — CASEpeer has no public REST API (verified via CASEpeer support docs: Zapier + nightly S3 sync only). Correct path = the existing generic webhook provider → Zapier → CASEpeer's Zapier app. Did NOT ship a fake adapter (§VIII honesty).
- **Ali sign-off:** Ali directed "ship filevine properly" (2026-07-20) — the §VII per-integration approval to commit the Filevine feature. Go-live still needs the human/§VII steps I did NOT do: the firm's Filevine PAT/orgId/userId + projectTypeId/custom-field map into `firm_integrations`, and flipping INTEGRATIONS_LIVE. No firm is connected, so the deploy transmits nothing.
- **Known bug logged:** `clio`/`casepeer` aren't in the `firm_integrations` provider CHECK constraint though `clio` has an adapter — adding providers needs a one-line migration (hosted apply).
- **Status:** Filevine shipped dormant; go-live pending Ali's env/secret steps. CASEpeer = webhook/Zapier path, no adapter.

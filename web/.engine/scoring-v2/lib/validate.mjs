...
// v2.2 additive fact catalog. OPTIONAL: shape-checked IF present, never
// required — so existing golds (which predate these) still validate, and a
// call that never surfaces them scores exactly as v2.1 (absence → unknown →
// abstain/develop; a new fact can never wrongly decline a case).
export const OPTIONAL_FACT_IDS = Object.freeze([
  "defendant_type",
  "coverage_stack",
  "objective_findings",
  "treatment_latency",
  "lien_sources",
  "rideshare_period",
  "public_entity_defendant",
  "commercial_truck",
  "defendant_dui",
  "prior_claims_same_region",
  "wrongful_death_survival",
  "medicare_status",
]);
...
export function validateLlmOutput(out) {
  ...
  if (!facts || typeof facts !== "object") {
    ...
    // Optional v2.2 facts: validate the envelope IF present; never required.
    for (const id of OPTIONAL_FACT_IDS) {
      const f = facts[id];
      if (f == null) continue;
      if (typeof f !== "object" || Array.isArray(f)) {
        errors.push(`optional fact ${id}: not an object`);
        continue;
      }
      if (!OBSERVABILITY.includes(f.observability))
        errors.push(`optional fact ${id}: bad observability ${f.observability}`);
      if (!isEvidence(f.evidence)) errors.push(`optional fact ${id}: bad evidence shape`);
      if (f.observability === "observed_on_call" && f.evidence === "checked, absent")
        errors.push(`optional fact ${id}: observed_on_call without a quote`);
    }
  }

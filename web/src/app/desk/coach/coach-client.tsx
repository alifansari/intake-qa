...
export function CoachClient() {
  ...
  // P0-2 (CIPA §632): the mic cannot open until the operator ACTIVELY confirms
  ...
  const startCall = async () => {
    // HARD GATE (P0-2 / CIPA §632): no mic without an active consent affirmation
    // AND a durable ConsentEvent recorded server-side. Block otherwise.
    if (!consentChecked) {
      setErr("Confirm all-party consent before starting.");
      return;
    }
    const SR = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
    if (!SR) { setMicState("unsupported"); return; }

    // Write the ConsentEvent first. If it fails, the mic never opens.
    setConsentBusy(true);
    try {
      const res = await fetch("/api/coach/consent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ affirmation: CONSENT_STATEMENT, confirmed: true }),
      });
      if (!res.ok) {
        setErr(
          res.status === 403
      ...
      setErr("Could not record consent. The call was not started.");
      setConsentBusy(false);
      return;
    }
    setConsentBusy(false);
  ...
  return (
    <div style={{ fontFamily: T.sans, color: T.ink, background: T.canvas, borderRadius: 12,
      ...
      {/* Persistent compliance banner (P0-2) — always visible, every state. */}
      <div role="note" style={{ display: "flex", alignItems: "center", gap: 8,
        padding: "9px 22px", background: T.amberTint, borderBottom: `1px solid ${T.line}`,
        color: T.amber, fontSize: 12.5, fontWeight: 600 }}>
        <span aria-hidden style={{ width: 7, height: 7, borderRadius: 999, background: T.amber, flexShrink: 0 }} />
        Demo — consented audio only, not a live claimant call.
      </div>
      ...
      {status === "idle" && (
        <div style={{ padding: "48px 22px", maxWidth: 620, margin: "0 auto", textAlign: "center" }}>
          ...
          <label style={{ display: "flex", alignItems: "flex-start", gap: 10, textAlign: "left",
            maxWidth: 540, margin: "0 auto 20px", padding: "14px 16px", background: T.paper,
            border: `1px solid ${consentChecked ? T.accent : T.lineStrong}`, borderRadius: 10,
            cursor: "pointer" }}>
            <input type="checkbox" checked={consentChecked}
              onChange={(e) => { setConsentChecked(e.target.checked); if (e.target.checked) setErr(""); }}
              style={{ marginTop: 3, width: 17, height: 17, flexShrink: 0, cursor: "pointer" }} />
            <span style={{ fontSize: 13, lineHeight: 1.5, color: T.ink }}>
              {CONSENT_STATEMENT}.
            </span>
          </label>
          ...
          <div>
            <button className="coach-btn"
              style={{ background: consentChecked ? T.accent : T.lineStrong, color: "#fff",
                cursor: consentChecked && !consentBusy ? "pointer" : "not-allowed", opacity: consentBusy ? 0.7 : 1 }}
              onClick={startCall} disabled={!consentChecked || consentBusy}
              aria-disabled={!consentChecked || consentBusy}>
              {consentBusy ? "Recording consent…" : "Start call"}
            </button>
          </div>
          ...
          {finalReport && (
            <div className="rise">
              ...
              <button className="coach-btn" style={{ background: T.navy, color: "#fff" }}
                onClick={() => { setStatus("idle"); setTranscript([]); setLive(null); setFinalReport(null); setErr(""); setConsentChecked(false); }}>
                New call
              </button>
            </div>
          )}
